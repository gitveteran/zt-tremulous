...
//ZT vmspoofer
static cvar_t        *ZT_vmSpoofer;
static int ZT_gamedir;
...
void ZT_gamedirOn( void )
{
	ZT_gamedir = 1;
}
void ZT_gamedirOff( void )
{
	ZT_gamedir = 0;
}
...
fileHandle_t FS_FOpenFileWrite( const char *filename ) {
	...
	if (ZT_gamedir == 1)
	{
		ospath = FS_BuildOSPath( fs_homepath->string, "ZT_data", filename );
	} else {
...
int FS_FOpenFileRead( const char *filename, fileHandle_t *file, qboolean uniqueFILE ) {
	...
	*file = FS_HandleForFile();
	...
	/*
	so spoofing happens like this
	-we find the pure cgame file, set is as a referenced file but dont really add it as a file handle
	-we find the ztqvm(in ZT_data folder in homepath) we add it to the avalable file handles but dont set it as referenced
	--what happends
		-pure cgame never gets used couse the handle is never added, instead our spoofed cgame gets used
		-when the client needs to send the md5's to the server it sends the pure's reference but the not spoofed
	--small problems
		sometimes the client saves demos and vids to the spoofed game dir, instead of the pure, and im not really sure why it does so
	*/
	
	if ( strstr(filename, "cgame.qvm") && ZT_vmSpoofer->integer )
	{
		for ( search = fs_searchpaths; search; search = search->next )
		{
			if ( search->pack ) {
			...
			if ( search->pack && search->pack->hashTable[hash] ) {
				...
				currentpakpure = FS_PakIsPure(search->pack);
				...
				do {
					if ( !FS_FilenameCompare( pakFile->name, filename ) ) {
						...
						if ( currentpakpure && (!foundpure) )
						{
							//set the pure qvm as referenced
							if (!(pak->referenced & FS_CGAME_REF) && strstr(filename, "cgame.qvm")) {
								//tell that we are using this pak.
								pak->referenced |= FS_CGAME_REF;
								foundpure = 1;//we have found the pure cgame
								if (foundZT)
								{
									Com_Printf("^5Using ZT VM Spoofer\n");
									return ZTlen;
								}
							}
						}
						if ( !Q_strncmp(pak->pakGamename, "ZT_data",7) )
						{

							if ( !Q_strncmp(pak->pakBasename, "ztqvm", 4 ) )
							{
								//use ztqvm as the handle
								foundZT = 1;
...
static void FS_Startup( const char *gameName )
{
	...
	//ZT ZT_vmSpoofer
	ZT_vmSpoofer = Cvar_Get("ZT_vmSpoofer", "1",1);
	...
	if (fs_homepath->string[0] && Q_stricmp(fs_homepath->string,fs_basepath->string)) {
		...
		//ZT stuff
		//add ZT game directory too.
		FS_AddGameDirectory ( fs_homepath->string, "ZT_data");
	}
