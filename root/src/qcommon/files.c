...
/*
===========
thz_FS_FOpenFileRead

v1 Updated on Aug 11, '07

Purpose:
 - This version of the FOpenFileRead function will bypass the pk3 files
   when loading qvms in such a way that the checksum originates from the
   pk3 and not the actual qvm.
...
int thz_FS_FOpenFileRead( const char *filename, fileHandle_t *file, qboolean uniqueFILE ) {
	...
	Com_Printf( "^3Reading [%s] via hacked open func\n", filename);
...
Hacked version of FS_ReadFile that ONLY reads from specific directory
============
*/

int thz_FS_ReadFile( const char *qpath, void **buffer ) {
	fileHandle_t	h;
	byte*			buf;
	qboolean		isConfig;
	int				len;
	
	//Com_Printf("^3 Stealth loading [%s]\n", qpath);
	...
	// look for it in the filesystem or pack files
	len = thz_FS_FOpenFileRead( qpath, &h, qfalse );
	if ( h == 0 ) {
		if ( buffer ) {
			*buffer = NULL;
		}
