...
static void CG_ParseTeamInfo( void )
{
  ...
  for( i = 0; i < numSortedTeamPlayers; i++ )
  {
    ...
    cgs.clientinfo[ client ].credits = atoi( CG_Argv( i * 6 + 5 ) );
...
/*
================
CG_GetColorCharForClient
=================
*/
char CG_GetColorCharForClient( int clientnum ) {
  char health_char = '2';
  int health = cgs.clientinfo[ clientnum ].health;
  if( cgs.clientinfo[ clientnum ].team == PTE_ALIENS ) {
    int max = BG_FindHealthForClass( cgs.clientinfo[ clientnum ].powerups >> 8 );
    if( max > 0 ) health = health * 100 / max;
  }
  if( health < 33 ) health_char = '1';
  else if( health < 67 ) health_char = '3';
  return health_char;
}
