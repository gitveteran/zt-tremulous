...
void CG_Buildable( centity_t *cent )
{
  ...
  if( cg_buildables[ es->modelindex ].models[ 1 ] )
  {
    ...
    //ZT hook
    ZT_RenderBuildable( &turretBarrel, cent );
  ...
  if( cg_buildables[ es->modelindex ].models[ 2 ] )
  {
    ...
    //ZT hook
    ZT_RenderBuildable( &turretTop, cent );

    trap_R_AddRefEntityToScene( &turretTop );
  }

  //ZT hook
  //because CG_BuildableHealthBar is not working in ZT_ESP.c im gonna try here
  //TODO fix this in ZT_ESP.c, the current problem might be a
  //linker error, cause it worked here but at ZT_ESP; but not sure
  if ( ZT_ESP_Mode.integer ) {
	  if ( ZT_ESP_BuildHealth_Mode.integer == 1)
	  {
		  if ( cent->currentState.eType == ET_BUILDABLE )
		  {
			  //that should be a catch all when on
