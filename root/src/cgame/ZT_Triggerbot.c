...
void ZT_TriggerAttack( void )
{
	...
	if ( cg.snap->ps.stats[ STAT_PTEAM] == PTE_HUMANS )
	{
		...
		else if ( cg.snap->ps.weapon == WP_HBUILD2){ return; }
		else{
	...
	{
		if ( cg.snap->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_BUILDER0 )
		{
			return;
		}
		//current player is a level 2 alien builder
		else if ( cg.snap->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_BUILDER0_UPG )
		{
		...
		//mara lightning
		else if (cg.snap->ps.stats[ STAT_PCLASS] == PCL_ALIEN_LEVEL2_UPG )
		{
...
void ZT_Triggerbot( void ){
	...
	else if (cent->currentState.eType == ET_BUILDABLE)
	{
		...
		if (cent->currentState.modelindex2 == BIT_ALIENS){ TargetTeam = 0;}
		else if( cent->currentState.modelindex2 == BIT_HUMANS ){ TargetTeam = 1;}
	}
	else if( cent->currentState.eType == ET_PLAYER )
	{
		...
		if( team == PTE_ALIENS ) { TargetTeam = 0; }
		else if( team == PTE_HUMANS ) { TargetTeam = 1; }
	}
